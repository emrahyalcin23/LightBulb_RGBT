...
function triggerUpdate() {
    ...
    if (isTime) {
        // Zaman modu: LT eğrisinden L, ardından T(t) ile RGB örnekle
        cL     = sampleAtX('LT', currentAmbient);
        effAmb = invertLSensor(cL);   // T(t) = L_sensor⁻¹(LT(t))
        cR     = sampleAtX('R', effAmb);
        cG     = sampleAtX('G', effAmb);
        cB     = sampleAtX('B', effAmb);
    } else {
        // Sensör modu: L_sensor, RGB doğrudan
...
if (calcBtn) {
    calcBtn.textContent = calcMode === 'ratio' ? 'Mod: Oran' : 'Mod: Mutlak';
